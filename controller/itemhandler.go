...
func ToUpdateBookPage(w http.ResponseWriter, r *http.Request) {
	...
	book, _ := service.GetItemByID(bookID) // 调取bookdao中获取图书的函数
	if book.ID > 0 {
...
func GetPageBooksByPrice(w http.ResponseWriter, r *http.Request) {
	...
	if minPrice == "" && maxPrice == "" {
		page, _ = service.GetPageItems(pageNo) // 调用bookdao中获取带分页的图书的函数
	} else {
		page, _ = service.GetPageItemsByPrice(pageNo, minPrice, maxPrice)
		page.MinPrice = minPrice // 将价格范围设置到page中
		page.MaxPrice = maxPrice
	}
	flag, session := service.IsLogin(r) // 调用IsLogin判断用户是否已经登录
	if flag {
